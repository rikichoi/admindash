  ...
  const stripeSubmitHandler = async (data: CreateDonationSchema) => {
    ...
    const result = await stripe.confirmPayment({
      ...
      confirmParams: {
        ...
        return_url: `http://localhost:3000/api/checkout/new-donation/${amount}/${orgId}/${encodeURIComponent(comment)}/${(donorName && encodeURIComponent(donorName)) || null}/${(itemId && donationType == "item" && itemId) || null}/${email || null}/${phone || null}/paymentId=asd`,
      },
    });
    console.log(result);
    if (result.error) {
