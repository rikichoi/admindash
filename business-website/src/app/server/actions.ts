...
export async function getOrganisations(): Promise<Organisation[] | undefined> {
    try {
        const organisations = (await axios.get(`http://${process.env.ENDPOINT_URL}/api/organisation/get-organisations`)).data
...
export async function getOrganisationItems(orgId: string): Promise<Item[] | undefined> {
    try {
        const organisations = (await fetch(`http://${process.env.ENDPOINT_URL}/api/item/get-org-items/${orgId}`)).json()
        return organisations
    } catch (error) {
        console.log(error)
    }
...
export async function getDonations(): Promise<Donation[] | undefined> {
    try {
        const donations = (await fetch(`http://${process.env.ENDPOINT_URL}/api/donation/get-donations`)).json()
        return donations
    } catch (error) {
        console.log(error)
    }
