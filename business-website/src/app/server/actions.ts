...
export async function getOrganisations(): Promise<Organisation[] | undefined> {
    try {
        const organisations = (await axios.get("http://3.128.24.35:5000/api/organisation/get-organisations")).data
...
export async function getOrganisationItems(orgId: string): Promise<Item[] | undefined> {
    try {
        const organisations = (await fetch(`http://3.128.24.35:5000/api/item/get-org-items/${orgId}`)).json()
        return organisations
    } catch (error) {
        console.log(error)
    }
